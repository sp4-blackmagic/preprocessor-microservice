    ...
    try:
        if cube_file is None:
            ...
            if not hasattr(hdr_file, "file"):
                raise InvalidFileFormatError(detail="The provided hdr file has no attribute 'file'")
            if not hasattr(cube_file, "file"):
                raise InvalidFileFormatError(detail="The provided cube file has no attribute 'file'")
